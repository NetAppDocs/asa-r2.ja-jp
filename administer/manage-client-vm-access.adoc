...
.ASA r2 システムでの LIF フェイルオーバー
コンポーネントの障害が発生した場合、LIF はフェイルオーバーするか、別の物理ポートに移行することで、ネットワーク通信が中断されないようにします。ONTAP 9.19.1 以降、iSCSI、NVMe/TCP、および共有 IP LIF に対して、自動 LIF フェイルオーバーがデフォルトでサポートされています。自動 LIF フェイルオーバーが有効になっている場合、ストレージ フェイルオーバーが発生すると、LIF は自動的にホーム ノードまたはポートから HA パートナー ノードまたはポートに移行され、フェイルオーバーが完了すると元のホーム ノードまたはポートに戻ります。あるいは、ポートが異常状態になった場合、LIF は自動的に現在のホーム ノード内の正常なポートに移行され、ポートが再び正常状態になると元のポートに戻ります。

ONTAP 9.18.1以前のバージョンでは、NVMe/TCP LIFまたは共有IP LIFに対する自動LIFフェイルオーバーはサポートされていません。iSCSI専用LIFを作成する場合、ストレージVMに既に自動LIFフェイルオーバーが有効になっていないiSCSI LIFが存在しない限り、自動LIFフェイルオーバーはデフォルトで有効になります。ストレージVM上に、自動LIFフェイルオーバーが有効になっていないiSCSI専用LIFが存在する場合、新しく作成されたLIFも有効になりません。このような場合は、まず既存のiSCSI専用LIFを削除し、その後、自動LIFフェイルオーバーを有効にして再作成する必要があります。link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html["SAN環境でLIFを削除する"^]方法をご確認ください。

永続ポートは、FCおよびNVMe/FC LIFのフェイルオーバーに使用されます。ONTAPドキュメントのlink:https://docs.netapp.com/us-en/ontap/san-config/asa-support-limitations.html#support-for-persistent-ports["永続ポート"^]の詳細をご覧ください。
...
ONTAP 9.18.1以前のバージョンでは、ストレージVM上の他のすべてのiSCSI LIFでも自動LIFフェイルオーバーが有効になっている場合、iSCSI専用LIFを作成すると、自動LIFフェイルオーバーがデフォルトで有効になります。ストレージVM上に、自動LIFフェイルオーバーが有効になっていないiSCSI専用LIFが存在する場合、新しく作成されたLIFも自動LIFフェイルオーバーが有効になりません。
...
.タスクの内容
ONTAP は、ネットワーク タイム プロトコル (NTP) を使用してクラスタ全体の時間を同期します。LIF IP アドレスを変更した後、同期の失敗を防ぐために NTP 設定を更新する必要がある場合があります。詳細については、ナレッジベースの記事を参照してください。link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-Issues/CONTAP-500629["LIF IPの変更後にNTP同期が失敗する"] 。
...
.関連情報
* link:https://docs.netapp.com/us-en/ontap/san-admin/move-san-lifs-task.html["SAN LIFの移動"]。
* link:https://docs.netapp.com/us-en/ontap/san-admin/remove-lif-from-portset-task.html["SAN LIF をポートセットから削除する"]。
