...
[role="lead"]
ONTAP 9.18.1 以降では、任意の標準 ASA クラスターから任意の ASA r2 クラスターへ、ストレージ仮想マシン（VM）を無停止で移行できます。標準 ASA クラスターから ASA r2 クラスターへ移行することで、SAN 専用環境向けの ASA r2 システムの簡素化・合理化されたアーキテクチャを採用できます。


NOTE: ASA MetroCluster から ASA r2 クラスターにストレージ VM を移行することはできません。ASA r2 システムは MetroCluster 構成をサポートしていません。
...
|===
ASAおよびASA r2システムの最新のリストについては、以下を参照してください。link:https://hwu.netapp.com/["NetApp Hardware Universe"^] 。  ASA r2 システムは、 NetApp Hardware Universeに「ASA A シリーズ/C シリーズ (新規)」として記載されています。
