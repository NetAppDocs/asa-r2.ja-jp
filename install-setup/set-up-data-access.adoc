...
ASA r2 ストレージ ユニットは、デフォルトでシン プロビジョニングされます。  VM ワークロードを移行する場合は、仮想ディスク (VMDK) もシン プロビジョニングする必要があります。
...
* 詳細はこちらlink:https://kb.netapp.com/on-prem/ontap/da/SAN/SAN-KBs/How_to_setup_Broadcom_VMware_ESXi_iSCSI_MPIO_with_ONTAP_SAN_ASA["ONTAP SAN ASAストレージシステムでBroadcom VMware ESXi iSCSI MPIOを設定する方法"^]。
...
ONTAP 9.19.1以降、FLIはバックエンド接続用のiSCSIプロトコルをサポートしています。

* リンクの詳細については、https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html[ONTAP FLI migrations^]を参照してください。
