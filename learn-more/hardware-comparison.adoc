...
searchtitle: ASA r2 とその他の ONTAP システムとの違い 
---
= ASA r2 とその他の ONTAP システムとの違い
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ストレージユニット（LUNまたはNVMeネームスペースに基づく）が作成されると、ONTAPは適切なストレージの可用性ゾーンにボリュームを自動的に作成し、ストレージユニットを格納します。新しく作成されたボリュームは、最適なパフォーマンスとバランスの取れた容量利用を実現するために、ストレージの可用性ゾーン内に自動的に配置されます。容量利用率は、ONTAPのバージョンに基づいて、ストレージの可用性ゾーン内でバランスが取られます。link:../administer/space-balancing.html["ASA r2 クラスタの容量バランスについて学習します"^]。
